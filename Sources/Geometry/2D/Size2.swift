...
public struct Size2<Length> where Length: Numeric & RawVectorizable2, Length.RawVector2: Equatable {

    /// The underlying vector, that stores the lengths and is masked by the point
    public private(set) var _vector: Vector2<Length>
    ...
    @_transparent public init(vector: Vector2<Length>) {
        _vector = vector
    }
    ...
    @_transparent public init(width: Length, height: Length) {
        self.init(vector: Vector2<Length>(index0: width, index1: height))
    }
}
...
extension Size2 where Length.RawVector2: AdditiveArithmetic {
    ...
    @_transparent public static func + (lhs: Size2, rhs: Vector2<Length>) -> Size2 {
    ...
    @_transparent public static func += (lhs: inout Size2, rhs: Vector2<Length>) {
        lhs._vector += rhs
    }
