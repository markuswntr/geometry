...
class Point2DTests: XCTestCase {

    func testZero() {
        let zero = Point2D<Int>.zero
        XCTAssertEqual(zero.x, 0)
        XCTAssertEqual(zero.y, 0)
    }

    func testEqual() {
        XCTAssertEqual(Point2D(x: 6, y: 4), Point2D(x: 6, y: 4))
        XCTAssertEqual(Point2D(x: 6.0, y: 4.5), Point2D(x: 6.0, y: 4.5))
        XCTAssertNotEqual(Point2D(x: 6, y: 4), Point2D(x: 4, y: 6))
    }

    func testComparable() {
        XCTAssertTrue(Point2D(x: 2, y: 0) < Point2D(x: 0, y: 3))
        XCTAssertFalse(Point2D(x: 6, y: 4) < Point2D(x: 2, y: 7))
    }
...
//    }
    ...
    func testSigning() {
        // In-place
        var p = Point2D(x: 3, y: -3)
        p.negate()
        XCTAssertEqual(p.x, -3)
        XCTAssertEqual(p.y, 3)
        // Copy
        XCTAssertTrue(Point2D(x: -2, y: 3) == -Point2D(x: 2, y: -3))
    }
